/* Tabled — availability windows and 30-minute slot generation (M6).
 *
 * A seller sets ONE standing weekly availability ("Saturdays 10am-2pm") that
 * applies across all their listings, not per-listing. That window gets sliced
 * into 30-minute increments a buyer can claim in one tap.
 *
 * ---- The timezone problem, and why this file is more than arithmetic ----
 *
 ...
 *
 * So: the seller's IANA zone is stored on their profile, windows are
 * interpreted in it, and everything is converted to a real instant before it's
 * stored or compared. Buyers see slots rendered in their own local time.
 *
 * Doing that without a date library needs the offset trick in zonedToUtc()
 * below. Note that JS Date's own parsing is no help — `new Date('2026-08-22
 * 10:00')` uses the *runtime's* zone, which is exactly the wrong one.
 *
 * ---- Mirroring ----
 *
 * This file is loaded by BOTH the browser and functions/timeslots.js, which is
 * a hand copy. `firebase deploy` only uploads functions/, so a require() that
 * reaches up into the repo works locally and fails in production. Change one,
 * change the other — the callable re-derives every instant the client sends, so
 * a divergence would show up as slots the server rejects for no visible reason.
 */
(function (root) {
